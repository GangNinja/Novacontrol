...
src/novacontrol/
  ...
  vision/         Vision pipeline: manager (OCR-first), replaceable provider, structured result
  models/         Model + hardware manager: capability registry (what each model
                  can DO, never its name), hardware monitor (RAM/CPU/GPU/NPU,
                  measured or reported as unmeasured), one place that answers
                  "which model, and is there room for it?"
...
close chrome                             → the same window close, for a NAMED app
open it (after opening Chrome)           → plans "open chrome": the resolved target reaches the plan
...
**Nothing is called done because it returned.** A step is COMPLETED only after its verification PASSED; a step that ran and could not be checked is reported **unverified**, which is different from a failure and very different from a success, and those unverified steps travel into the final answer instead of being rounded up. Verification is required by policy for every side-effecting action and optional anywhere else, reads evidence rather than opinions (a file, an exit code, captured output, an observed change, a named check) and returns **inconclusive** when it genuinely cannot tell — the honest answer, and not a soft pass. A nonzero exit code is a *result* when the goal was "tell me what failed", so the run for a test suite accepts any exit code while still capturing it for the analysis. **A plan's own command steps run for real — and only with approval.** `run-tests` and `run-command` steps are carried out by the step handler: the project is located first, its runner is *recognised* (`npm test`, else `pytest` from the interpreter that owns the process — anything unrecognised is reported, not guessed at), the command is executed **exec-form with no shell**, output is captured from the tail where a test summary actually is, a hard wall-clock timeout stops a hung process, and the exit code is handed to the deterministic verifier. The handler refuses with a permission reason unless the caller named the step in `run_plan(approved=...)` (the same `approved` list `POST /plan/run` exposes), so the chat path asks for approval instead of stopping at an internal error, and an approved run completes and verifies.
...
**Vision is its own capability, and it reads before it interprets.** A picture attached to a request is read *instead of* the screen — `POST /ask` takes an optional `image` path — and its presence is a fact the understanding layer is told rather than left to infer, so *"what is this?"* with a picture needs eyes while the same words without one stay the plain question they are. A screenshot is not handed to the text model — a separate pipeline owns it: capture → **`VisionManager`** → **`VisionProvider`** → structured **`VisionResult`** → the planner. Each image is first read by OCR (cheap, deterministic, and on this build the *same* Windows OCR the desktop already uses, geometry included), and the model is consulted only when the text genuinely cannot answer: *"read the text in this screenshot"* and *"what is this error?"* are answered from the text, and so is *"where is the login button?"* whenever the read words carry positions — a located label's own coordinates **are** the answer, and paying a VLM for a point the read already produced is exactly the inference this layer exists to avoid. It escalates only when the label was not placed (a plain text file knows its words and not where they were drawn) or not found at all. The gate is measured rather than assumed — the result's confidence is the fraction of the question's content words the text actually contains — and the task is read from the question itself, so the cheap path is chosen by meaning rather than by a caller remembering to ask for it. **The provider is a plug, not a dependency**: `VisionProvider` is one method (`see(prompt, image_path)`), configuration decides which model sits behind it (`vision.provider` / `vision.model`, plus `NOVACONTROL_VISION_*`), the capability gate still refuses a text-only chat model rather than trusting it to invent coordinates, and `set_provider` hot-swaps at runtime — so Qwen3-VL, Gemma vision, another local VLM or a cloud endpoint is a wiring change, and tests drive the whole pipeline with a fake provider. What leaves is structured and bounded: `image_type`, `detected_text`, `ui_elements`, `errors`, `relevant_regions`, `summary`, `confidence`, plus provenance under `metadata` — which reader answered, whether the model was consulted (including when it was consulted and *failed*), and whether the question was really answered. No hidden reasoning travels, and `answered: false` with a reason is a real outcome — no model wired, an unreadable image, or a question that needs eyes. The structured result then travels beside the plan, so a follow-up is compiled against what was **seen** (the error lines, the elements) rather than against prose the planner would have to re-read. Computer use is designed for and not enabled: `VisionActionProposal` is the fourth step of the screenshot → understand → target → proposal → approve → click → screenshot → verify chain, `requires_approval` is a constant no caller can flip, no geometry means no proposal, and no function in it clicks anything.
...
- **One model does not have to handle everything (Phase 7)** — `models/` declares what each model can *do* (`text`, `reasoning`, `coding`, `vision`, `tool_calling`, `structured_output`, `embeddings`, context length) and selects by matching a requirement against those declarations, so `qwen3:8b` appears once in the codebase as data and never as a branch. The runtime's own `/api/show` capability report is applied last and in the directions it can be trusted: a capability it confirms is added (an undeclared VLM pulled five minutes ago becomes selectable for a screenshot), a capability it can express and does *not* list is removed (a text-only build whose name suggests vision is not trusted with an image), and silence changes nothing. Loading is RAM-aware — check memory, inspect what is resident, estimate the footprint (measured when the runtime knows it), unload **only what must go and never a model an active task is using**, load, then verify that the runtime lists the model — and a refusal is a result that names the numbers, not an exception. The switch the specification describes (the chat model resident, a screenshot needing a VLM, room made for it) happens on the request path and not only behind an operator's load button, and when the measurement says the vision model will not fit the pipeline answers from the screen's text and says so. Residency policies (`immediate`, `warm`, `while_active`, `never`) are applied by the handlers that use a model: a chat answer marks its model in use for as long as it is being written, so an eviction cannot take the weights mid-sentence.
...
| Model + hardware manager (Phase 7) | ✅ Working — `models/` selects a model by **capability** rather than by name, loads it RAM-aware (measure → inspect → estimate → evict only what must go → load → verify), refuses with the numbers when it will not fit rather than paging, and keeps the model resident under a configurable policy; the runtime's own `/api/show` capability report corrects both declarations and name guesses, so an undeclared VLM that was pulled five minutes ago is selectable while a text-only build named like a vision model is not; the cloud fallback is preserved as a *route*, and no accelerator is claimed that was not detected. Every residency policy is applied by the handler that uses a model (the vision pipeline acquires the VLM before it looks, and a refusal downgrades to OCR with the reason in the answer), the specification's four example flows are covered end to end — including the multi-clause one, which reaches the planner and its tools rather than being answered from its first clause in 57 ms instead of 44 s — and provider timings are filed under the call that produced them. Measured on the dev box: capability probe 0.058 s, a 3.07 GB model loads and verifies in 7.3–8.0 s, unload 31 ms, NPU reported as undetected with the reason |
| Vision pipeline (Phase 6) | ✅ Working — a dedicated `VisionManager` reads every image through OCR first and escalates to the configured VLM only when the text cannot answer, returning a structured `VisionResult` (`image_type`, `detected_text`, `ui_elements`, `errors`, `relevant_regions`, `summary`, `confidence`) with its provenance; the text chat model is never handed an image, the provider is configuration-chosen and hot-swappable, and a machine with no vision model declines honestly instead of inventing a description |
...
- The file operations (`find_file`, `read_file`, `list_files`, `write_file`, …) and the device controls (`volume_control`, `brightness_control`, `media_control`) are *understood* but have no executor wired yet, so they fall through to the existing handlers; a plan's command and test steps execute only when the caller approves those step ids (an unapproved run reports the refusal rather than attempting it)
